...
/**
 * Transport for SIP over secure WebSocket (WSS), adapted for Node.js.
 * @public
 */
export class Transport implements TransportDefinition {
    ...
    private keepAliveDebounceTimeout: NodeJS.Timeout | undefined;
    ...
    private _connect(): Promise<void> {
        ...
        try {
            ...
            // isn't exact W3C API, so convert as needed:
            ws.onclose = (ev: ICloseEvent) => this.onWebSocketClose(ev, ws);
            ws.onerror = (err: Error) => this.onWebSocketError(err, ws);
            ws.onopen = () => this.onWebSocketOpen(ws);
            ws.onmessage = (ev: IMessageEvent) => this.onWebSocketMessage(ev, ws);
            this._ws = ws;
        } catch (error) {
            this._ws = undefined;
     ...
     * @param ev - Event.
     */
    private onWebSocketClose(ev: ICloseEvent, ws: WebSocket): void {
        if (ws !== this._ws) {
            return;
        }
     ...
     * @param ev - Event.
     */
    private onWebSocketError(err: Error, ws: WebSocket): void {
        if (ws !== this._ws) {
            return;
        }
     ...
     * @param ev - Event.
     */
    private onWebSocketMessage(ev: IMessageEvent, ws: WebSocket): void {
        if (ws !== this._ws) {
            return;
        }
